...
from .fixtures.login_fixture import auth_token
from .fixtures.create_news import create_news


logger = logging.getLogger(__name__)

def test(create_news):
    logger.info(create_news)
...
def test_get_summary_by_id(eco_news_id, expected_param, expected_response, expected_schema, token, auth_token, create_news):
    ...
    assert response.json()[expected_param]==expected_response.get(expected_param), f"Expected {expected_param}: {expected_response.get(expected_param)} byt got {response.json()[expected_param]}"
